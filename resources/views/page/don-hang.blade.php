...
@section('content')
    <div class="container" style="margin-top: 5%;margin-bottom: 5%">
        <div class="col-sm-7" style="background-color: white;padding: 20px;margin-left:40px"  >
            ...
            <div class="col-sm-10">
                <form  action="{{route('post_don_hang')}}" method="post">
                    ...
                    <input style="ma" class="btn btn-primary" type="submit" value="Đặt hàng">
                </form>
            </div>

        </div>
        <div class="col-sm-4" style="background-color: white;margin-left: 20px">
            <div class="alert-info col-sm-12 " style="text-align: center">
            ...
            <ul class="fa-ul col-sm-8" style="background: white;margin-top: 20px">
                ...
                @foreach($sp as $c)
                    <li class="row" style="margin-bottom: 20px">
                        ...
                        <div class="col-sm-6" style="margin-left: 25px">
                            <div class="li-name">
                                <p style="font-size: 15px">{{$c->name}}</p>
                            </div>
                            <div class="li-price">
                                @if($c->amount > 1)
                                    @if($c->sale_price != 0)
                                        <p style="font-size: 15px;margin-top: -5px">{{$c->sale_price * $c->amount }} SL:{{$c->amount}}</p>

                                    @else
                                        <p style="font-size: 15px;margin-top: -5px">{{$c->unit_price *$c->amount}} SL:{{$c->amount}}</p>

                                    @endif
                                @else
                                    @if($c->sale_price != 0)
                                        <p style="font-size: 15px;margin-top: -5px">{{$c->sale_price  }}</p>

                                    @else
                                        <p style="font-size: 15px;margin-top: -5px">{{$c->unit_price }}</p>
                        ...
                        <div  style="position: absolute; left: 26%;top:5%;">
                ...
                @endforeach
                    ...
                    @else
                        <div style="text-align: center">TỔNG TIỀN : {{number_format($total)}} đồng</div>
                    @endif
